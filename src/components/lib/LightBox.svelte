...
<div class="relative">
	<button class="bg-dynamic-neutral-200 fill-dynamic-neutral-800 border border-dynamic-neutral-500 p-3 rounded-md mt-6 mr-6 absolute right-0" on:click={toggleLightbox} aria-label="Open lightbox">
...
<div class={`fixed inset-0 bg-dynamic-neutral-0 flex items-center justify-center z-50 ${showLightbox ? "flex" : "hidden"}`}>
	<button class="bg-dynamic-neutral-200 fill-dynamic-neutral-800 border border-dynamic-neutral-500 p-3 rounded-md mt-6 mr-6 absolute top-0 right-0" on:click={toggleLightbox} aria-label="Close lightbox">
		<svg xmlns="http://www.w3.org/2000/svg" class="size-6" viewBox="0 0 16 16">
			<path
